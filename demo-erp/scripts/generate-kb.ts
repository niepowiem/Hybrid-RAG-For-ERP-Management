...
/**
 * Moduł wyprowadzamy z prefiksu kodu, zamiast wpisywać na sztywno: pula
 * ERR-9xxx należy do CRM, reszta do magazynu. Dzięki temu dołożenie kolejnego
 * modułu nie wymaga pamiętania o tym pliku — wystarczy trzymać się numeracji.
 */
const modulDlaKodu = (code: string): string => (code.startsWith("ERR-9") ? "crm" : "magazyn");

const docs: KbError[] = Object.values(ERRORS).map((e) => ({
  id: e.code,
  module: modulDlaKodu(e.code),
